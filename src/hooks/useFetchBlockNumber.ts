...
export const useFetchBlockNumber = () => {
  ...
  useEffect(() => {
    let unsubscribe: any; // We'll use this to unsubscribe from the block number updates when the component unmounts.
    ...
    const watchBlockNumber = async () => {
      ...
      if (api) {
        try {
          ...
          unsubscribe = observable.subscribe((newBlockNumber: any) => {
          ...
          setError(e instanceof Error ? e.message : "Failed to watch block number");
          setLoading(false);
        }
      }
    };
