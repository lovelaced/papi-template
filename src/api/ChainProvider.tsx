...
import { createClient } from 'polkadot-api'; // Function to create a Polkadot API client
...
interface ChainContextType {
  api: any | null; // The API object from Polkadot that will be used to interact with the blockchain. It's `null` initially.
...
export const ChainProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [api, setApi] = useState<any | null>(null); // Holds the Polkadot API instance.
  ...
  return (
    <ChainContext.Provider value={{ api, loading, error }}>
      {children} {/* `children` are the components that are wrapped by ChainProvider, like BlockNumberDisplay. */}
